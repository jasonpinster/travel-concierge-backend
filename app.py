...
def plan_trip():
    ...
    return jsonify({"itinerary": itinerary, "dining_recommendations": dining_recommendations})

@app.route("/search_hotels", methods=["GET"])
def search_hotels():
    location = request.args.get("location", "Melbourne")
    token = get_amadeus_token()
    url = f"https://test.api.amadeus.com/v1/reference-data/locations?keyword={location}&subType=CITY"
    headers = {"Authorization": f"Bearer {token}"}
    city_data = requests.get(url, headers=headers).json()
...
def search_flights():
    ...
    token = get_amadeus_token()
    url = f"https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=MEL&destinationLocationCode={destination}&departureDate=2025-09-10&adults=1"
    headers = {"Authorization": f"Bearer {token}"}
    flight_data = requests.get(url, headers=headers).json()
    flights = [f"{f['itineraries'][0]['segments'][0]['carrierCode']} - {f['price']['total']} AUD" for f in flight_data.get("data", [])]
